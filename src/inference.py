...
def get_model_predictions(model, features: pd.DataFrame) -> pd.DataFrame:
    # past_ride_columns = [c for c in features.columns if c.startswith['ridess]]
...
def load_batch_of_features_from_store(current_date: datetime) -> pd.DataFrame:
    ...
    Args:
        current_date (datetime): dtaetime of the prediction for which we want to get the batch of features
    ...
    #read the time-series data from the feature_store
    ...
    #transpose time_series data as a feature vector for each pickup_location_id
    x = np.array(shape = (len(location_ids), n_features), dtype=np.float32)
    ...
    features = features.sort_values(by = ['pickup_location_id'])
